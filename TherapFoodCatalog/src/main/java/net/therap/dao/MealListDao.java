...
import net.therap.controller.MealRow;
...
public class MealListDao {
    ...
    public void addMealOfADay(String mealType, String dateToAdd, long[] foodsToAdd, long userId) {
        for (long aFoodsToAdd : foodsToAdd) {
            new DatabaseTemplate().executeInsertQuery("INSERT INTO meal_list_tmp(id,f_id,m_type,u_id,day_added)" +
                    " VALUES(?,?,?,?,?)", 0, aFoodsToAdd, mealType, userId, dateToAdd);
        }
    }

    public Map<String, MealCompact> getBothMealListOfLastMonth() {

        List<MealRow> mealRowList = new DatabaseTemplate().queryForObject("SELECT f_id, m_type, day_added, f_name " +
                "FROM meal_list_tmp, food_list WHERE meal_list_tmp.f_id=food_list.id " +
                "AND day_added BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE() ",
                new ObjectRowMapper<MealRow>() {
                    public MealRow mapRowToObject(ResultSet resultSet) throws SQLException {
                        MealRow mealRow = new MealRow();
                        mealRow.setMealType(resultSet.getString("m_type"));
        ...
        return makeCompactMealRowFromMealRowList(mealRowList);
    }
    ...
    private Map<String, MealCompact> makeCompactMealRowFromMealRowList(List<MealRow> mealRowList) {
        ...
        while (mealRowIterator.hasNext()) {
            ...
            String dayString = DateHelper.getDayNameFromDateString(addedDate);
